...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // RANDOM USER BUTTON - Bot চালু করে Main UI তে নিয়ে যায়
    ...
    if (randomUserBtn) {
        randomUserBtn.addEventListener("click", function () {
            ...
            statusDiv.innerHTML = `<span style="color:#dc143c;font-size:15px;font-weight:600;"><i class="fas fa-spinner fa-spin"></i> Random Bot সংযুক্ত হচ্ছে... (~30s)</span>`;
            ...
            .then(data => {
                ...
                if (data.status === 'success' || data.status === 'already_online') {
                    statusDiv.innerHTML = `<span style="color:#00ff88;font-size:15px;font-weight:600;">✓ Bot চালু! UID: ${data.uid}</span>`;
                    LOGGED_IN_BOT_UID = data.uid;
                    localStorage.setItem('bot_uid', data.uid);
                    setTimeout(() => showMainUI(), 800);
                } else {
                    statusDiv.innerHTML = `<span style="color:#ff4444;font-size:15px;font-weight:600;">✗ ${data.message || 'Failed'}</span>`;
                }
            })
            .catch(e => {
                randomUserBtn.disabled = false;
                randomUserBtn.style.opacity = "1";
                statusDiv.innerHTML = `<span style="color:#ff4444;font-size:15px;font-weight:600;">✗ Server error: ${e.message}</span>`;
            });
        });
    }
