...
# Guest account cycling index (চক্রাকারে ব্যবহার হয়)
GUEST_ACCOUNTS_LIST = list(GUEST_ACCOUNTS.items())  # [(uid, pw), ...]
GUEST_CYCLE_INDEX = 0
...
def join_team():
    ...
    if not bot_uid:
        return jsonify({"status": "error", "message": "bot_uid দিন"})

    # Bot চালু না থাকলে এখনই start করো (lazy start)
    if bot_uid not in user_bots:
        password = GUEST_ACCOUNTS.get(bot_uid)
        if not password:
            return jsonify({"status": "error", "message": "Bot চালু নেই। আগে Login করুন।"})
        future = asyncio.run_coroutine_threadsafe(StartUserBot(bot_uid, password), _global_loop)
        try:
            bot_data_new, msg = future.result(timeout=40)
            if bot_data_new.get('status') == 'online':
                user_bots[bot_uid] = bot_data_new
                print(f"[Join] Lazy-started bot uid={bot_uid}")
            else:
                return jsonify({"status": "error", "message": f"Bot চালু করা যায়নি: {msg}"})
        except Exception as e:
            return jsonify({"status": "error", "message": f"Bot সংযোগ ব্যর্থ: {str(e)[:80]}"})
...
def random_auto():
    """চক্রাকার guest account বেছে bot চালু করে — শেষ হলে আবার প্রথম থেকে"""
    global GUEST_CYCLE_INDEX, GUEST_ACCOUNTS_LIST, _global_loop

    if not GUEST_ACCOUNTS_LIST:
        return jsonify({"status": "error", "message": "কোনো guest account নেই"})

    total = len(GUEST_ACCOUNTS_LIST)
    # সর্বোচ্চ total বার চেষ্টা করো — একটা valid account পাওয়া পর্যন্ত
    for attempt in range(total):
        uid_str, password = GUEST_ACCOUNTS_LIST[GUEST_CYCLE_INDEX % total]
        GUEST_CYCLE_INDEX = (GUEST_CYCLE_INDEX + 1) % total

        # ইতিমধ্যে চালু ও online থাকলে সরাসরি ব্যবহার করো
        if uid_str in user_bots and user_bots[uid_str].get('status') == 'online':
            print(f"[random-auto] Reusing online bot uid={uid_str}")
            return jsonify({"status": "already_online", "message": "Bot চালু আছে!", "uid": uid_str})

        # নতুন করে start করো
        print(f"[random-auto] Starting uid={uid_str} (cycle {GUEST_CYCLE_INDEX}/{total})")
        future = asyncio.run_coroutine_threadsafe(StartUserBot(uid_str, password), _global_loop)
        try:
            bot_data, msg = future.result(timeout=35)
            if bot_data.get('status') == 'online':
                user_bots[uid_str] = bot_data
                return jsonify({"status": "success", "message": "Random Bot চালু হয়েছে!", "uid": uid_str})
            else:
                print(f"[random-auto] uid={uid_str} failed: {msg}, trying next...")
                continue
        except Exception as e:
            print(f"[random-auto] uid={uid_str} exception: {e}, trying next...")
            continue

    return jsonify({"status": "error", "message": "সকল account সাময়িকভাবে অনুপলব্ধ। পরে চেষ্টা করুন।"})

@app.route('/api/random-pick', methods=['POST'])
def random_pick():
    """Instantly returns a cycling guest uid — no bot startup, চক্রাকার"""
    global GUEST_CYCLE_INDEX, GUEST_ACCOUNTS_LIST
    if not GUEST_ACCOUNTS_LIST:
        return jsonify({"status": "error", "message": "কোনো guest account নেই"})
    total = len(GUEST_ACCOUNTS_LIST)
    uid_str, _ = GUEST_ACCOUNTS_LIST[GUEST_CYCLE_INDEX % total]
    GUEST_CYCLE_INDEX = (GUEST_CYCLE_INDEX + 1) % total
    print(f"[RandomPick] Issued uid={uid_str} (index={GUEST_CYCLE_INDEX-1}/{total})")
